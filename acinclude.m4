...
AC_ARG_ENABLE(drivers,
	...
	[                    lcterm,icp_a106,ms6931,century]
	[                  \"all\" compiles all drivers],
  	drivers="$enableval",
  	drivers=[lcdm001,mtxorb,cfontz,cfontz633,curses,cwlnx,text,lb216,bayrad,glk])

if test "$drivers" = "all"; then
	drivers=[mtxorb,cfontz,cfontz633,curses,cwlnx,text,lb216,mtc_s16209x,hd44780,joy,irman,lirc,bayrad,glk,stv5730,sed1330,sed1520,svga,lcdm001,t6963,lcterm,icp_a106,ms6931,century]
fi
	...
	case "$driver" in
		...
		bayrad)
			...
			;;
		century)
			DRIVERS="$DRIVERS century${SO}"
			actdrivers=["$actdrivers century"]
			;;			
		cfontz)
